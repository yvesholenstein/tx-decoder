...
function readOptionalTxMeta(prefix, defaultToAddress = null) {
    ...
    if (!nonceValue || !chainIdValue || !gasLimitValue || !toValue) {
        return null;
    }
    ...
    try {
        const meta = {
            ...
            nonce: ethers.BigNumber.from(nonceValue),
            chainId: ethers.BigNumber.from(chainIdValue),
            gasLimit: ethers.BigNumber.from(gasLimitValue),
            value: ethers.BigNumber.from(valueValue || '0'),
            to: ethers.utils.getAddress(toValue)
        };

        if (meta.type === 'legacy') {
            const gasPriceValue = document.getElementById(`${prefix}-tx-gasprice`)?.value.trim();
            if (!gasPriceValue) return null;
            ...
            if (!maxFeeValue || !maxPriorityValue) return null;
...
async function decodeManual() {
    ...
    try {
        ...
        if (decodedWithBaseAbi) {
            ...
            displayDecoded(decodedWithBaseAbi);
            return;
        }
        ...
        if (explorerImplementation) {
            ...
            if (decodedExplorer) {
                ...
                displayDecoded(decodedExplorer);
                return;
            }
        }
...
async function displayDecoded(decoded) {
    ...
    html += `<div class="info-row">
        ...
        <div class="info-value">
            ...
            ${isApproval ? '<div class="small-text">You\'re giving someone else permission to spend this amount from your account.</div>' : ''}
        </div>
    </div>`;
    ...
    if (bridgeSummary) {
        ...
        if (bridgeSummary.parameters && bridgeSummary.parameters.length > 0) {
            ...
            bridgeSummary.parameters.forEach(param => {
                const rawValue = param.value !== undefined && param.value !== null
                    ? param.value
                    : 'Not available';
